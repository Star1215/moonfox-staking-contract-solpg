...
export async function setStakePoolRewards(stakePool: web3.PublicKey, admin: web3.Signer, rewardsPerSecond: anchor.BN, periodTimestamp: anchor.BN) {
  let accounts = await getSetStakePoolRewardsAccounts(admin.publicKey, stakePool);
  const tx = await program.methods
    .setStakePoolRewards(rewardsPerSecond, periodTimestamp)
    .accounts(accounts)
    .signers([admin])
    .rpc();
...
export async function createStakePoolWithRewards(creator: web3.Signer, mintA: web3.PublicKey, mintB: web3.PublicKey, rewardsPerSecond: anchor.BN, periodTimestamp: anchor.BN, mintBAuthority: web3.Signer = null) {
  ...
  return await setStakePoolRewards(stakePool, creator, rewardsPerSecond, periodTimestamp);
}
